...
class RedisDataManagerException(Exception):
    """Custom exception for RedisDataManager errors"""
    """Custom exception for RedisDataManager errors. Extend for more granular error handling."""

class DatasetCacheModel(BaseModel):
    id: str
    name: str
...
class DatasetQualityMetricsModel(BaseModel):
    overall_score: float
...
class RedisDataManager:
    ...
    def __init__(self) -> None:
        """Initialize RedisDataManager instance"""
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            try:
                ...
                # Connection failed; mark as not initialized but do not raise so
                # application can continue without Redis in local/dev setups.
                logger.error(f"Failed to connect to Redis (non-fatal): {e}")
                self.client = None
                self.initialized = False
                return

            # Initialize data structures only when Redis is available
            try:
                await self._initialize_data_structures()
                self.initialized = True
                logger.info("Redis Data Manager initialized successfully")
            except Exception as e:
                logger.error(f"Failed to initialize Redis data structures: {e}")
                # Keep initialized False but don't raise to avoid crashing startup
                self.initialized = False
                return
    ...
    async def _initialize_data_structures(self) -> None:
    ...
    async def _create_user_schema(self) -> None:
    ...
    async def _create_session_schema(self) -> None:
    ...
    async def _create_fl_schema(self) -> None:
    ...
    async def _create_security_schema(self) -> None:
    ...
    async def _create_monitoring_schema(self) -> None:
    ...
    async def _create_cache_schema(self) -> None:
    ...
    async def _create_task_schema(self) -> None:
    ...
    async def store_user_profile(self, user_id: str, profile_data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            string_data = {k: str(v) for k, v in profile_data.items()}
            profile_data['updated_at'] = datetime.now(timezone.utc).isoformat()

            # Use individual hset calls for Redis 3.2 compatibility
            for field, value in string_data.items():
    ...
    async def store_user_session(self, session_id: str, user_id: str, session_data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            # Convert all values to strings and use individual hset calls
    ...
    async def queue_task(self, task_type: str, task_data: Dict[str, Any], priority: int = 0) -> Optional[str]:
        ...
        try:
            ...
            # Store task data using individual hset calls
    ...
    async def backup_data(self, backup_key: str) -> bool:
        ...
        try:
            ...
            # Use individual hset calls
            for field, value in backup_info.items():
                self.client.hset(f"backup:{backup_key}", field, value)
...
# Global Redis data manager instance
    ...
    # --- Dataset Management Extensions ---
    async def list_datasets_paginated(self, page: int = 1, per_page: int = 20) -> Tuple[List[DatasetCacheModel], int]:
        """List datasets in Redis (paginated)"""
        keys = self.client.keys("cache:api:dataset:*")
        datasets = []
        for key in keys:
            data = self.client.hgetall(key)
            if data and "data" in data:
                dataset = json.loads(data["data"])
                datasets.append(DatasetCacheModel(**dataset))
        total_items = len(datasets)
        start_idx = (page - 1) * per_page
        end_idx = start_idx + per_page
        return datasets[start_idx:end_idx], total_items

    async def delete_dataset_atomic(self, dataset_id: str) -> bool:
        """Delete dataset and all related keys atomically"""
        try:
            pipe = self.client.pipeline()
            pipe.delete(f"cache:api:dataset:{dataset_id}")
            pipe.delete(f"cache:api:dataset:{dataset_id}:lineage")
            pipe.delete(f"cache:api:dataset:{dataset_id}:quality")
            pipe.execute()
            return True
        except Exception as e:
            logger.error(f"Atomic delete failed: {e}")
            raise RedisDataManagerException(f"Atomic delete failed: {e}")

    async def store_dataset_lineage(self, parent_id: str, child_id: str, transformation: Dict[str, Any]) -> bool:
        """Store dataset lineage info"""
        try:
            lineage = DatasetLineageModel(
                parent_id=parent_id,
                child_id=child_id,
                transformation=transformation,
                created_at=datetime.now(timezone.utc).isoformat()
            )
            key = f"cache:api:dataset:{parent_id}:lineage"
            self.client.rpush(key, lineage.json())
            return True
        except Exception as e:
            logger.error(f"Failed to store lineage: {e}")
            return False

    async def get_dataset_lineage(self, dataset_id: str) -> List[DatasetLineageModel]:
        """Get dataset lineage info"""
        key = f"cache:api:dataset:{dataset_id}:lineage"
        lineage_jsons = self.client.lrange(key, 0, -1)
        return [DatasetLineageModel.parse_raw(j) for j in lineage_jsons]

    async def store_quality_metrics(self, dataset_id: str, metrics: Dict[str, Any]) -> bool:
        """Store dataset quality metrics"""
        try:
            qm = DatasetQualityMetricsModel(
                overall_score=metrics.get("overall_score", 0.0),
                analysis=metrics.get("analysis", {}),
                last_updated=datetime.now(timezone.utc).isoformat()
            )
            key = f"cache:api:dataset:{dataset_id}:quality"
            self.client.set(key, qm.json())
            return True
        except Exception as e:
            logger.error(f"Failed to store quality metrics: {e}")
            return False

    async def get_quality_metrics(self, dataset_id: str) -> Optional[DatasetQualityMetricsModel]:
        """Get dataset quality metrics"""
        key = f"cache:api:dataset:{dataset_id}:quality"
        qm_json = self.client.get(key)
        if qm_json:
            return DatasetQualityMetricsModel.parse_raw(qm_json)
        return None
